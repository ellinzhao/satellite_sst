...
class MMRecon(nn.Module):
    ...
    def __init__(self, chs=[24, 32, 48, 64], feat_ch=32):
        ...
        self.mw_encoder = Encoder(in_ch=1, proj_ch=feat_ch, chs=chs)
        self.ir_encoder = Encoder(in_ch=1, proj_ch=feat_ch * 2, chs=chs)

        self.lr_decoder = Decoder(chs=chs)
        self.hr_decoder = Decoder(chs=chs)
